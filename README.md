...
Repo này bám trực tiếp vào yêu cầu “Trợ lý ảo thông minh hỗ trợ phân tích và truy xuất thông tin chuyên sâu trong dữ liệu lớn multimedia”:

- Multimedia retrieval: xử lý hình ảnh/keyframe, OCR text, transcript ASR, caption/VLM tags.
- Vietnamese-focused processing: Whisper `large-v3` với Vietnamese prompt, OCR `en,vi`, query tiếng Việt dịch VI->EN cho SigLIP2 visual search.
- Traditional mode: UI tương tác để search, xem nearby frames, pin frame nghi ngờ đúng và submit `videoId + timeMs`.
- Automatic mode ready: `/api/agent/solve` trả query plan + candidates cho thử nghiệm assistant tự động, mặc định không auto-submit để tránh nộp nhầm.
- LVLM/GenAI-ready: caption/VLM tags và optional rerank/VLM rerank có thể thêm ở stage top candidates.
- Competition submission contract: backend gửi `answerSets.answers[].mediaItemName/start/end`, trong đó `start=end=timeMs` dạng string.
...
```powershell
python -m scripts.benchmark_latency --url http://localhost:5000
```
...
Session/evaluation không cần hardcode lại rồi restart. UI có thanh `Evaluation` để dán:
...
Bấm `Save` sẽ lưu vào `localStorage` và sync vào backend runtime qua:

```http
POST /api/evaluation-config
```

Sau đó các lần submit dùng ngay session/evaluation mới. Backend vẫn có fallback từ `.env` nếu UI chưa nhập gì.
